...
- スレッド内スラッシュコマンド
  - `/cwd [path]`: スレッド内ではそのスレッドの作業ディレクトリを表示または設定。変更時はセッションもリセット
  - `/model [id]`: スレッド内ではそのスレッドのモデルを表示または設定。通常チャンネルでは新規スレッド向けのデフォルトモデルを表示または設定
  - `/reset`: セッションと作業ディレクトリを初期化
  - `/status`: 直近利用状況に加えて、作業ディレクトリ・モデル・セッション情報を表示
- 通常チャンネルで `/cwd [path]` を実行すると、そのチャンネルで新しく作るスレッド向けのデフォルト作業ディレクトリを設定
- 永続化ファイル
  - `.state.json`: 管理中スレッド、セッション情報、作業ディレクトリ設定
