...
function isHighRiskOperation(toolName: string, input: Record<string, unknown>): boolean {
  if (toolName === 'Bash') {
  ...
  if (toolName === 'Edit') {
    ...
    return Boolean(grantRoot) || /\bdelete\b|\boverwrite\b|\brename\b/i.test(reason);
  }
...
export function createApprovalManager() {
  ...
  function resolveApproval(requestId: string, decision: ToolApprovalDecision): boolean {
